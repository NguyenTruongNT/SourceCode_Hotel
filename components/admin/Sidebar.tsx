    ...
    <aside className="w-64 bg-[#0F172A] text-white min-h-screen border-r border-slate-800 flex flex-col shrink-0 transition-all duration-300 shadow-xl z-20">
      <div className="h-[70px] px-6 flex items-center border-b border-white/5">
        <h1 className="text-2xl font-bold tracking-wide">
          THAD<span className="font-light text-blue-500">HOTEL</span>
        </h1>
      </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`w-full flex items-center space-x-3 px-4 py-3.5 rounded-lg font-medium transition-all duration-200 ${
                isActive
