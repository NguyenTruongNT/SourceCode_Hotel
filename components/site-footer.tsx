import Link from "next/link"

export function SiteFooter() {
  return (
    <footer className="border-t border-border bg-card">
      <div className="mx-auto grid max-w-6xl gap-8 px-4 py-12 sm:px-6 md:grid-cols-4">
        <div className="md:col-span-1">
          <div className="flex items-center gap-2">
            <span className="flex size-8 items-center justify-center rounded-md bg-primary text-sm font-bold text-primary-foreground">
              T
            </span>
            <span className="text-lg font-semibold text-foreground">THAD HOTEL</span>
          </div>
          <p className="mt-4 text-sm leading-relaxed text-muted-foreground">
            Kiến tạo không gian nghỉ dưỡng đẳng cấp và trải nghiệm ẩm thực tinh hoa tại trung tâm thành phố.
          </p>
        </div>
        <div>
          <h3 className="text-sm font-semibold text-foreground">Liên kết nhanh</h3>
          <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
            <li>
              <Link href="/" className="hover:text-foreground">
                Về chúng tôi
              </Link>
            </li>
            <li>
              <Link href="/search" className="hover:text-foreground">
                Phòng nghỉ
              </Link>
            </li>
            <li>
              <Link href="/" className="hover:text-foreground">
                Sự kiện
              </Link>
            </li>
          </ul>
        </div>
        <div>
          <h3 className="text-sm font-semibold text-foreground">Chính sách</h3>
          <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
            <li>
              <Link href="/" className="hover:text-foreground">
                Chính sách bảo mật
              </Link>
            </li>
            <li>
              <Link href="/" className="hover:text-foreground">
                Điều khoản sử dụng
              </Link>
            </li>
            <li>
              <Link href="/" className="hover:text-foreground">
                Quy định đặt phòng
              </Link>
            </li>
          </ul>
        </div>
        <div>
          <h3 className="text-sm font-semibold text-foreground">Liên hệ</h3>
          <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
            <li>123 Đường Du Lịch, Quận 1, TP. Hồ Chí Minh, Việt Nam</li>
            <li>Hotline: (+84) 123 456 789</li>
            <li>Email: contact@thadhotel.com</li>
          </ul>
        </div>
      </div>
      <div className="border-t border-border">
        <p className="mx-auto max-w-6xl px-4 py-4 text-center text-xs text-muted-foreground sm:px-6">
          © 2026 THAD HOTEL. All rights reserved.
        </p>
      </div>
    </footer>
  )
}
